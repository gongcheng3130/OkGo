...
import com.cheng.okgo.http.fileserver.OkDownload;
import com.cheng.okgo.http.fileserver.task.PriorityRunnable;
import com.cheng.okgo.http.okgo.db.DownloadManager;
...
import com.cheng.okgo.http.okgo.utils.IOUtils;
...
public class DownloadTask implements Runnable {
    ...
    public void start() {
        ...
        if (progress.status == Progress.NONE || progress.status == Progress.PAUSE || progress.status == Progress.ERROR) {
            ...
            if (progress.filePath == null) {
                postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE, "the file of the task with tag:" + progress.tag + " may be invalid or damaged, please call the method restart() to download again！"));
            } else {
                File file = new File(progress.filePath);
                if (file.exists() && file.length() == progress.totalSize) {
                    postOnFinish(progress, new File(progress.filePath));
                } else {
                    postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE, "the file " + progress.filePath + " may be invalid or damaged, please call the method restart() to download again！"));
                }
            }
        } else {
    ...
    public void run() {
        ...
        if (startPosition < 0) {
            postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE, "breakpoint file has expired!"));
            return;
        }
        if (startPosition > 0) {
            if (!TextUtils.isEmpty(progress.filePath)) {
                File file = new File(progress.filePath);
                if (!file.exists()) {
                    postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE, "breakpoint file does not exist!"));
                    return;
                }
            }
        }
        ...
        if (code == 404 || code >= 500) {
            postOnError(progress, new MyResponseException(MyErrorCode.RESPONSE_OTHER, "network error! http response code is 404 or 5xx!"));
            return;
        }
        ResponseBody body = response.body();
        if (body == null) {
            postOnError(progress, new MyResponseException(MyErrorCode.RESPONSE_OTHER, "response body is null"));
            return;
        }
        ...
        if (!IOUtils.createFolder(progress.folder)) {
            postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE
                    , "SDCard isn't available, please check SD card and permission: WRITE_EXTERNAL_STORAGE, and you must pay attention to Android6.0 RunTime Permissions!"));
            return;
        }
        ...
        if (progress.status == Progress.PAUSE) {
            ...
            postOnError(progress, new MyResponseException(MyErrorCode.CUSTOM_STORAGE, "unknown exception!"));
        }
    }
